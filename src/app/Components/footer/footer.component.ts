import { Component } from '@angular/core';
import { CommonModule } from '@angular/common';

@Component({
  selector: 'app-footer',
  standalone: true,
  imports: [CommonModule],
  template: `
    <footer class="text-gray-600 body-font">
      <div
        class="container px-5 py-24 mx-auto flex md:items-center lg:items-start md:flex-row md:flex-nowrap flex-wrap flex-col"
      >
        <div
          class="w-64 flex-shrink-0 md:mx-0 mx-auto text-center md:text-left md:mt-0 mt-10"
        >
          <a
            class="flex title-font font-medium items-center md:justify-start justify-center text-gray-900"
          >
            <img
              src="assets/logo-images/official-rapture-ready.svg"
              alt="logo-image"
            />
            <span class="ml-3 text-l">Official Rapture Ready</span>
          </a>
        </div>
        <div
          class="flex-grow flex flex-wrap -mb-10 md:pr-20 md:text-left text-center order-first"
        >
          <!-- Resource Links Section 1 -->
          <div class="lg:w-1/4 md:w-1/2 w-full px-4 mb-10">
            <div class="p-6 rounded-lg shadow-lg bg-white">
              <h2
                class="title-font font-semibold text-gray-900 tracking-widest text-md mb-3"
              >
                RESOURCES
              </h2>
              <nav class="list-none mb-10">
                <li>
                  <a
                    href="https://www.biblegateway.com"
                    class="text-indigo-600 hover:text-indigo-800 transition-colors duration-300"
                  >
                    Bible Gateway
                  </a>
                </li>
                <li>
                  <a
                    href="https://www.desiringgod.org"
                    class="text-indigo-600 hover:text-indigo-800 transition-colors duration-300"
                  >
                    Desiring God
                  </a>
                </li>
                <li>
                  <a
                    href="https://www.christianitytoday.com"
                    class="text-indigo-600 hover:text-indigo-800 transition-colors duration-300"
                  >
                    Christianity Today
                  </a>
                </li>
                <li>
                  <a
                    href="https://www.crosswalk.com"
                    class="text-indigo-600 hover:text-indigo-800 transition-colors duration-300"
                  >
                    Crosswalk
                  </a>
                </li>
              </nav>
            </div>
          </div>
          <!-- Resource Links Section 2 -->
          <div class="lg:w-1/4 md:w-1/2 w-full px-4 mb-10">
            <div class="p-6 rounded-lg shadow-lg bg-white">
              <h2
                class="title-font font-semibold text-gray-900 tracking-widest text-md mb-3"
              >
                STUDY
              </h2>
              <nav class="list-none mb-10">
                <li>
                  <a
                    href="https://www.blueletterbible.org"
                    class="text-indigo-600 hover:text-indigo-800 transition-colors duration-300"
                  >
                    Blue Letter Bible
                  </a>
                </li>
                <li>
                  <a
                    href="https://www.ligonier.org"
                    class="text-indigo-600 hover:text-indigo-800 transition-colors duration-300"
                  >
                    Ligonier Ministries
                  </a>
                </li>
                <li>
                  <a
                    href="https://www.gotquestions.org"
                    class="text-indigo-600 hover:text-indigo-800 transition-colors duration-300"
                  >
                    Got Questions
                  </a>
                </li>
                <li>
                  <a
                    href="https://www.biblehub.com"
                    class="text-indigo-600 hover:text-indigo-800 transition-colors duration-300"
                  >
                    Bible Hub
                  </a>
                </li>
              </nav>
            </div>
          </div>
          <!-- Resource Links Section 3 -->
          <div class="lg:w-1/4 md:w-1/2 w-full px-4 mb-10">
            <div class="p-6 rounded-lg shadow-lg bg-white">
              <h2
                class="title-font font-semibold text-gray-900 tracking-widest text-md mb-3"
              >
                COMMUNITY
              </h2>
              <nav class="list-none mb-10">
                <li>
                  <a
                    href="https://www.christianforums.com"
                    class="text-indigo-600 hover:text-indigo-800 transition-colors duration-300"
                  >
                    Christian Forums
                  </a>
                </li>
                <li>
                  <a
                    href="https://www.worthyboards.com"
                    class="text-indigo-600 hover:text-indigo-800 transition-colors duration-300"
                  >
                    Worthy Christian Forums
                  </a>
                </li>
                <li>
                  <a
                    href="https://www.christiandiscourse.net"
                    class="text-indigo-600 hover:text-indigo-800 transition-colors duration-300"
                  >
                    Christian Discourse
                  </a>
                </li>
                <li>
                  <a
                    href="https://www.christiansocialnetwork.net"
                    class="text-indigo-600 hover:text-indigo-800 transition-colors duration-300"
                  >
                    Christian Social Network
                  </a>
                </li>
              </nav>
            </div>
          </div>
          <!-- Resource Links Section 4 -->
          <div class="lg:w-1/4 md:w-1/2 w-full px-4 mb-10">
            <div class="p-6 rounded-lg shadow-lg bg-white">
              <h2
                class="title-font font-semibold text-gray-900 tracking-widest text-md mb-3"
              >
                INSPIRATION
              </h2>
              <nav class="list-none mb-10">
                <li>
                  <a
                    href="https://www.dailyverses.net"
                    class="text-indigo-600 hover:text-indigo-800 transition-colors duration-300"
                  >
                    Daily Verses
                  </a>
                </li>
                <li>
                  <a
                    href="https://www.ibelieve.com"
                    class="text-indigo-600 hover:text-indigo-800 transition-colors duration-300"
                  >
                    iBelieve
                  </a>
                </li>
                <li>
                  <a
                    href="https://www.klove.com"
                    class="text-indigo-600 hover:text-indigo-800 transition-colors duration-300"
                  >
                    K-LOVE
                  </a>
                </li>
                <li>
                  <a
                    href="https://www.lightsource.com"
                    class="text-indigo-600 hover:text-indigo-800 transition-colors duration-300"
                  >
                    LightSource
                  </a>
                </li>
              </nav>
            </div>
          </div>
        </div>
      </div>
      <div class="bg-gray-100">
        <div
          class="container mx-auto py-4 px-5 flex flex-wrap flex-col sm:flex-row"
        >
          <p class="text-gray-500 text-sm text-center sm:text-left">
            © 2023 Official Rapture Ready
          </p>
          <span
            class="inline-flex sm:ml-auto sm:mt-0 mt-2 justify-center sm:justify-start"
          >
            <a class="text-gray-500">
              <svg
                fill="currentColor"
                stroke-linecap="round"
                stroke-linejoin="round"
                stroke-width="2"
                class="w-5 h-5"
                viewBox="0 0 24 24"
              >
                <path
                  d="M18 2h-3a5 5 0 00-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 011-1h3z"
                ></path>
              </svg>
            </a>
            <a class="ml-3 text-gray-500">
              <svg
                fill="currentColor"
                stroke-linecap="round"
                stroke-linejoin="round"
                stroke-width="2"
                class="w-5 h-5"
                viewBox="0 0 24 24"
              >
                <path
                  d="M23 3a10.9 10.9 0 01-3.14 1.53 4.48 4.48 0 00-7.86 3v1A10.66 10.66 0 013 4s-4 9 5 13a11.64 11.64 0 01-7 2c9 5 20 0 20-11.5a4.5 4.5 0 00-.08-.83A7.72 7.72 0 0023 3z"
                ></path>
              </svg>
            </a>
            <a class="ml-3 text-gray-500">
              <svg
                fill="none"
                stroke="currentColor"
                stroke-linecap="round"
                stroke-linejoin="round"
                stroke-width="2"
                class="w-5 h-5"
                viewBox="0 0 24 24"
              >
                <rect width="20" height="20" x="2" y="2" rx="5" ry="5"></rect>
                <path
                  d="M16 11.37A4 4 0 1112.63 8 4 4 0 0116 11.37zm1.5-4.87h.01"
                ></path>
              </svg>
            </a>
            <a class="ml-3 text-gray-500">
              <svg
                fill="currentColor"
                stroke="currentColor"
                stroke-linecap="round"
                stroke-linejoin="round"
                stroke-width="0"
                class="w-5 h-5"
                viewBox="0 0 24 24"
              >
                <path
                  stroke="none"
                  d="M16 8a6 6 0 016 6v7h-4v-7a2 2 0 00-2-2 2 2 0 00-2 2v7h-4v-7a6 6 0 016-6zM2 9h4v12H2z"
                ></path>
                <circle cx="4" cy="4" r="2" stroke="none"></circle>
              </svg>
            </a>
          </span>
        </div>
      </div>
    </footer>
  `,
})
export class FooterComponent {}
